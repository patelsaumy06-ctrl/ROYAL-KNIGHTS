...
import { findAccount, DEMO_ACCOUNTS, addAccount, emailExists, NGO_TYPES } from '../services/api';
...
function SignIn({ onLogin, onGoSignUp }) {
  ...
  return (
    <div style={{minHeight:"100vh",width:"100vw",display:"flex", flexDirection: isMobile ? "column" : "row", fontFamily:"'Inter',sans-serif",overflow:"hidden"}}>
      {showBrandPanel && (
        <div style={{
          flex: isTablet ? "0 0 38%" : "0 0 45%", background:"linear-gradient(160deg, #0B1120 0%, #0F172A 50%, #1E293B 100%)",
          ...
          <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{duration:0.8}} style={{position:"relative",zIndex:1,textAlign:"center",maxWidth:420}}>
            <div style={{
              width:72,height:72,borderRadius:20,
              background:"linear-gradient(135deg,#2563EB,#8B5CF6)",
              display:"inline-flex",alignItems:"center",justifyContent:"center",
              marginBottom:32,fontSize:34,
              boxShadow:"0 20px 40px rgba(37,99,235,0.3), inset 0 1px 0 rgba(255,255,255,0.2)"
            }}>🤝</div>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",background:"#FAFCFF",padding: isMobile ? "40px 20px" : 40,position:"relative", minHeight: isMobile ? "100vh" : "auto"}}>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.2}} style={{width:"100%",maxWidth:420,position:"relative",zIndex:1}}>
          {isMobile && (
          ...
          <form onSubmit={handleLogin}>
            <div style={{marginBottom:20}}>
              ...
              <div style={{position:"relative"}}>
                <Mail size={17} color={focusField==="email"?"#2563EB":"#94A3B8"} style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",transition:"color 0.2s",pointerEvents:"none"}}/>
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@ngo.org" required
                  ...
                  style={{
                    ...
                    border:`1.5px solid ${error?"#FECACA":focusField==="email"?"#2563EB":"#E2E8F0"}`,
                    borderRadius:12,fontSize:14,fontFamily:"'Inter',sans-serif",color:"#0F172A",
                    outline:"none",transition:"all 0.2s ease",background:error?"#FEF2F2":"#fff",
                    boxShadow:focusField==="email"?`0 0 0 4px rgba(37,99,235,0.08)`:"",
                    minHeight: 48,
                  }}/>
              </div>
            </div>
            ...
            <div style={{marginBottom:12}}>
              ...
              <div style={{position:"relative"}}>
                <Lock size={17} color={focusField==="pass"?"#2563EB":"#94A3B8"} style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",transition:"color 0.2s",pointerEvents:"none"}}/>
                <input type={showPwd?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Enter your password" required
                  ...
                  style={{
                    ...
                    border:`1.5px solid ${error?"#FECACA":focusField==="pass"?"#2563EB":"#E2E8F0"}`,
                    borderRadius:12,fontSize:14,fontFamily:"'Inter',sans-serif",color:"#0F172A",
                    outline:"none",transition:"all 0.2s ease",background:error?"#FEF2F2":"#fff",
                    boxShadow:focusField==="pass"?`0 0 0 4px rgba(37,99,235,0.08)`:"",
                    minHeight: 48,
                  }}/>
            ...
            <button type="submit" disabled={loading}
              style={{
                ...
                fontFamily:"'Inter',sans-serif",transition:"all 0.25s cubic-bezier(0.16,1,0.3,1)",
            ...
            >
              {loading ? <><div style={{width:16,height:16,border:"2.5px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/>Signing in…</> : <>Sign In to Portal <ArrowRight size={18}/></>}
            </button>
          </form>

          <div style={{textAlign:"center",marginTop:28,fontSize:14,color:"#64748B"}}>
            Don't have an account?{" "}
            <span onClick={onGoSignUp} style={{color:"#2563EB",fontWeight:700,cursor:"pointer"}}>
              Create one →
            </span>
          </div>
          ...
          <div style={{marginTop:32,padding:"16px 20px",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:12}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {DEMO_ACCOUNTS.slice(0,3).map(a=>(
                <div key={a.email} onClick={()=>{setEmail(a.email);setPassword(a.password)}} style={{fontSize:12,color:"#475569",cursor:"pointer",display:"flex",justifyContent:"space-between",padding:"6px 0",transition:"color 0.15s", minHeight: 32, alignItems: "center"}}
...
function SignUp({ onSignedUp, onGoSignIn }) {
  ...
  const validate = () => {
    ...
    if (emailExists(form.email)) e.email = "An account with this email already exists";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false);
    setSuccess(true);
    setTimeout(() => onSignedUp(newAccount), 1400);
  };
  ...
  if (success) return (
    <div style={{minHeight:"100vh",width:"100vw",background:"linear-gradient(135deg,#EFF6FF 0%,#F0FDF4 50%,#FFFBEB 100%)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif",padding: isMobile ? "0 20px" : 0}}>
      <div style={{textAlign:"center",padding: isMobile ? 24 : 40,animation:"popIn 0.5s ease"}}>
  ...
  return (
    <div style={{minHeight:"100vh",width:"100vw",background:"linear-gradient(135deg,#EFF6FF 0%,#F0FDF4 50%,#FFFBEB 100%)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif",padding: isMobile ? "24px 16px" : "32px 0"}}>
      <div style={{width:"100%",maxWidth:500,padding: isMobile ? "0" : "0 24px",animation:"fadeUp 0.35s ease"}}>
        ...
        <div style={{background:"#fff",borderRadius: isMobile ? 16 : 20,border:"1px solid #E8EDF5",padding: isMobile ? "24px 18px" : "36px 32px",boxShadow:"0 20px 60px rgba(0,0,0,0.07)"}}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{position:"relative"}}>
                ...
                <input type="text" {...inp("name")} placeholder="e.g. Priya Sharma" required
                  style={{width:"100%",padding:"11px 12px 11px 38px",border:"1.5px solid #E2E8F0",borderRadius:10,fontSize:14,fontFamily:"'Inter',sans-serif",color:"#0F172A",outline:"none",transition:"all 0.2s",minHeight:44}}/>
              </div>
              {fieldErr("name")}
            </div>
              ...
              <div>
                ...
                <div style={{position:"relative"}}>
                  <select value={form.orgType} onChange={e=>set("orgType",e.target.value)}
                    style={{width:"100%",padding:"11px 12px 11px 12px",border:"1.5px solid #E2E8F0",borderRadius:10,fontSize:14,fontFamily:"'Inter',sans-serif",color:"#0F172A",outline:"none",background:"#fff",appearance:"none",cursor:"pointer",minHeight:44}}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{position:"relative"}}>
                ...
                <input type={showPwd?"text":"password"} {...inp("password")} placeholder="Min. 6 characters" required
                  style={{width:"100%",padding:"11px 40px 11px 38px",border:"1.5px solid #E2E8F0",borderRadius:10,fontSize:14,fontFamily:"'Inter',sans-serif",color:"#0F172A",outline:"none",transition:"all 0.2s",minHeight:44}}/>
                {eyeBtn("password",showPwd,setShowPwd)}
              </div>
            ...
            <div style={{marginBottom:24}}>
              ...
              <div style={{position:"relative"}}>
                ...
                <input type={showCfm?"text":"password"} {...inp("confirm")} placeholder="Re-enter your password" required
                  style={{width:"100%",padding:"11px 40px 11px 38px",border:"1.5px solid #E2E8F0",borderRadius:10,fontSize:14,fontFamily:"'Inter',sans-serif",color:"#0F172A",outline:"none",transition:"all 0.2s",minHeight:44}}/>
                {eyeBtn("confirm",showCfm,setShowCfm)}
              </div>
              {fieldErr("confirm")}
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><div style={{width:14,height:14,border:"2px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 0.7s linear infinite"}}/>Creating account…</> : "Create Account →"}
            </button>
          </form>

          <div style={{textAlign:"center",marginTop:22,fontSize:13,color:"#64748B"}}>
            Already have an account?{" "}
            <span onClick={onGoSignIn} style={{color:"#2563EB",fontWeight:600,cursor:"pointer"}}>
              Sign in →
            </span>
          </div>
        </div>
...
export default function Auth({ onLogin }) {
  ...
  const handleSignedUp = (account) => {
    onLogin(account);
  };

  if (mode === 'signup') {
    return <SignUp onSignedUp={handleSignedUp} onGoSignIn={() => setMode('signin')} />;
  }
